...
if [ "$build" == 1 ]; then
    ...
    if which datadog-ci >/dev/null ; then
        ...
        datadog-ci sourcemaps upload services/frontend/statics/js --service vulcan-app --release-version 1.11 --minified-path-prefix /js/ | grep --line-buffered "^Uploading sourcemap*" | sed "s/^/  /"
