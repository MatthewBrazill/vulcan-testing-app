...
method:"POST",data:{query:$("#god-search-bar").val()},beforeSend:searchWait,success:searchSuccess,error:searchError}))});$("#divination-input").keypress(a=>{"13"==a.which&&(console.log(`asking for a prediction for: ${$("#divination-input").val()}`),a.preventDefault(),$.ajax({url:"/oracle/predict",method:"POST",data:{question:$("#divination-input").val(),oracle:$("#divination-dropdown").val()},beforeSend:()=>{$("#divination-output").attr("class","");$("#divination-output-loader").attr("class","ui active text loader");
$("#divination-output-text").attr("class","ui hidden")},success:b=>{$("#divination-output").attr("class","");$("#divination-output-loader").attr("class","ui hidden text loader");$("#divination-output-text").text(b.prediction);$("#divination-output-text").attr("class","ui active")},error:b=>{$("#divination-output").attr("class","");$("#divination-output-loader").attr("class","ui hidden text loader");$("#divination-output-text").text(b.prediction);$("#divination-output-text").attr("class","ui active")}}))});
$("#testing-log-input").keypress(a=>{if("13"==a.which){a.preventDefault();a=$("#testing-log-input").val();var b=$("#testing-log-dropdown").val();switch(b){case "console":console.log(a);$("#testing-log-input").val("");break;case "datadog":$("#testing-log-input").val("");(window.DD_LOGS&&window.DD_LOGS.logger).info(a);break;default:throw console.error(`unrecognized logging method: ${b}`),Error(`Didn't recognize logging method: ${b}`);}}});$("#testing-error-button").click(()=>{console.log("generating testing error");
$.ajax({url:"https://random-word-api.herokuapp.com/word?number=3",method:"GET",success:a=>{throw Error(`Example error: ${a[0]} - ${a[1]} - ${a[2]}`);},error:a=>console.error(`error request failed: ${a.status} - ${a.message}`)})});$("#testing-custom-event-button").click(()=>{console.warn("warning for custom action button");window.DD_RUM&&window.DD_RUM.addAction("custom-testing-action",{value:"testing"})})});
function searchWait(a){for(;2<$("#god-list").children().length;)$("#god-list").children().last().remove();$("#god-list").attr("class","");$("#god-list-loader").attr("class","ui active text loader");$("#god-list-error").attr("class","ui hidden error message")}function searchSuccess(a){if(0<a.result.length)for(var b of a.result)godItem=`
